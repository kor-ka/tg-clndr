...
export const renderEvent = async ({ date, tz, title, description, attendees, deleted, geo, chatId, endDate }: SavedEvent, options?: { timeZones?: Set<string>, renderDate?: boolean, renderAttendees?: boolean, atChat?: { name: string, id: number } }) => {
    ...
    const endTimeStr = endDate ? ` - ${new Date(endDate).toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit', hourCycle: 'h24', timeZone: tz })}` : '';
