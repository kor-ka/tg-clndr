...
export const renderEvent = async ({ date, tz, title, description, attendees, deleted, geo }: SavedEvent, options?: { timeZones?: Set<string>, renderDate?: boolean, renderAttendees?: boolean, atChat?: { name: string, id: number } }) => {
    ...
    const dateStr = renderDate !== false ? `🗓️ ${new Date(date).toLocaleString('en-Gb', { month: 'short', day: 'numeric', timeZone: tz })} - ` : '';
    const timeStr = new Date(date).toLocaleTimeString('en-Gb', { hour: '2-digit', minute: '2-digit', hourCycle: 'h24', timeZone: tz });
